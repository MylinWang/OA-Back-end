...
import com.gelin.oa.mapper.LeaveFormMapper;
import com.gelin.oa.mapper.ProcessFlowMapper;
import com.gelin.oa.pojo.Employee;
import com.gelin.oa.pojo.LeaveForm;
import com.gelin.oa.pojo.ProcessFlow;
import com.gelin.oa.service.exception.LeaveFormException;
...
public class LeaveFormService {
    ...
    public LeaveForm createLeaveForm(LeaveForm form){
        LeaveForm leaveForm = (LeaveForm) MybatisUtil.executeUpdate(sqlSession -> {
    ...
    public List<Map> getLeaveFormList(String pfState,Long pfOperatorId){
        return (List<Map>)MybatisUtil.executeQuery(sqlSession -> {
            LeaveFormMapper mapper = sqlSession.getMapper(LeaveFormMapper.class);
            List<Map> maps = mapper.selectByParams(pfState, pfOperatorId);
            return maps;
        });
    }
    ...
    public void audit(Long formId,Long operatorId,String result,String reason){
        MybatisUtil.executeUpdate(sqlSession -> {
            ...
            ProcessFlowMapper processFlowMapper=sqlSession.getMapper(ProcessFlowMapper.class);
            List<ProcessFlow> formList = processFlowMapper.selectByFormId(formId);
            if(formList.size()==0){
                throw new LeaveFormException("无效的审批流程");
            }
            //获取当前任务ProcessFlow对象
            List<ProcessFlow> processList = formList.stream().filter(p -> p.getOperatorId() == operatorId && p.getState().equals("process")).collect(Collectors.toList());
            ...
            LeaveFormMapper mapper = sqlSession.getMapper(LeaveFormMapper.class);
            LeaveForm leaveForm = mapper.selectById(formId);
            //如果当前任务是最后一个节点，代表流程结束，更新请假单状态为对应的approved/refused
            if (process.getIsLast()==1){
                leaveForm.setState(result);
                mapper.update(leaveForm);
            }else{
                ////readyList包含所有后续任务节点
                List<ProcessFlow> readyList = formList.stream().filter(p -> p.getState().equals("ready")).collect(Collectors.toList());
